...
const updatePostule = asyncHandler(async (req, res, next) => {
...
// for dashboard admin
const deletePostule = asyncHandler(async (req, res, next) => {
    const deleteNews = await Newsletter.findById(req.params.id);

    if (!deleteNews)
        throw createError(404, `not found with id of ${req.params.id}`);
